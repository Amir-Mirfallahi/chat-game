...
if dotenv_path.exists():
    ...
    print(
        f"Warning: .env file not found at {dotenv_path}. Using default settings or environment variables."
    )
...
SECRET_KEY = os.getenv(
    "DJANGO_SECRET_KEY",
    "django-insecure-h0^3t%i%_%c4a2t$k@t8_l2e62!c*o()rn(+atfhm%ty7=d38h",
...
if (
    not ALLOWED_HOSTS
):  # Ensure ALLOWED_HOSTS is not empty, especially if DJANGO_ALLOWED_HOSTS is not set
...
SIMPLE_JWT = {
    "ACCESS_TOKEN_LIFETIME": timedelta(
        minutes=int(os.getenv("DJANGO_JWT_ACCESS_TOKEN_LIFETIME_MINUTES", "60"))
    ),
    "REFRESH_TOKEN_LIFETIME": timedelta(
        days=int(os.getenv("DJANGO_JWT_REFRESH_TOKEN_LIFETIME_DAYS", "7"))
    ),
    ...
    "SLIDING_TOKEN_LIFETIME": timedelta(
        minutes=5
    ),  # Not typically used with access/refresh pairs
...
if (
    SENTRY_DSN and SENTRY_DSN.strip()
):  # Ensure DSN is not None and not an empty/whitespace string
    import sentry_sdk
...
SPECTACULAR_SETTINGS = {
    ...
    "DESCRIPTION": "API for the Language Learning Environment (LLE) backend, providing services for user management, speech processing generation.",
    "VERSION": "1.0.0",
...
EMAIL_PORT = int(os.getenv("EMAIL_PORT", 587))  # Manual casting is needed

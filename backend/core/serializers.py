...
class ReportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Report
        fields = ["id", "child", "summary", "description", "score", "created_at"]
        read_only_fields = ["id", "created_at"]

    def validate_child(self, value):
        """
        Ensure the child belongs to the requesting user when creating a report.
        This assumes the serializer context has the request object.
        """
        request = self.context.get("request")
        if request and hasattr(request, "user"):
            if not Report.objects.filter(
                child__user=request.user, pk=value.pk
            ).exists():
                raise serializers.ValidationError(
                    "You can only create reports for your own child's sessions."
                )
        return value

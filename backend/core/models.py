...
class Child(models.Model):
    ...
    native_language = models.CharField(max_length=10, help_text="ISO 639-1 language code (e.g., 'en', 'es', 'fa')")
...
class Session(models.Model):
    ...
    child = models.ForeignKey(Child, on_delete=models.CASCADE, related_name='sessions', db_index=True)
    livekit_room = models.CharField(max_length=100, unique=True, help_text="Unique name for the LiveKit room")
    started_at = models.DateTimeField(auto_now_add=True)
    ended_at = models.DateTimeField(null=True, blank=True, help_text="Timestamp when the session ended")

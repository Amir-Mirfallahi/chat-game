...
from .models import Child, Session, Report
...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        if hasattr(user, "child_profile"):
            return Session.objects.filter(child=user.child_profile).order_by(
                "-started_at"
            )
...
class ReportViewSet(
    ...
    """
    Create, retrieve and list reports for a child's sessions.
    - POST /api/reports/ -> create a new report for a child's session.
    - GET /api/reports/ -> list all reports for the authenticated user's child.

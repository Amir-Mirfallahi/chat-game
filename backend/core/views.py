...
from .serializers import (
    ...
    ChildSerializer,
    SessionSerializer,
    SessionCreateResponseSerializer # This was defined in core.serializers
)
...
@method_decorator(ratelimit(key='ip', rate=getattr(settings, 'RATELIMIT_AUTH_REGISTER_RATE', '10/h'), method='POST', block=True), name='dispatch')
...
@method_decorator(ratelimit(key='ip', rate=getattr(settings, 'RATELIMIT_AUTH_LOGIN_RATE', '10/m'), method='POST', block=True), name='dispatch')
...
@method_decorator(ratelimit(key='ip', rate=getattr(settings, 'RATELIMIT_TOKEN_REFRESH_RATE', '20/m'), method='POST', block=True), name='dispatch')
...
class ChildViewSet(viewsets.ModelViewSet): # Changed to ReadOnlyModelViewSet initially
    ...
    # class ChildViewSet(viewsets.ReadOnlyModelViewSet, UpdateModelMixin):
    # ... then implement partial_update / update methods or rely on mixin.
    # For now, keeping it ReadOnly as per initial interpretation of spec.
...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        if hasattr(user, 'child_profile'):
            return Session.objects.filter(child=user.child_profile).order_by('-started_at')
    ...
    def end_session(self, request, pk=None):
        ...
        if session.child.user != request.user:
             return Response({"error": "You do not have permission to end this session."}, status=status.HTTP_403_FORBIDDEN)
